...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("[deshold/api/log]", error)
      ...
      void (async () => {
        try {
          ...
          if (insertErr?.code === "23505") {
            ...
            console.error("[deshold] dm_activity_sessions insert:", insertErr.message)
          }
        } catch (dbErr) {
          console.error("[deshold] dm_activity_sessions error:", dbErr)
        }
      })()
    }
    ...
    console.error("[deshold/api/log]", err)
