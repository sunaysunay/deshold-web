...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("[desgroup/api/log]", error)
      ...
      void (async () => {
        try {
          ...
          if (insertErr?.code === "23505") {
            ...
            console.error("[desgroup] activity_log2 insert:", insertErr.message)
          }
        } catch (dbErr) {
          console.error("[desgroup] activity_log2 error:", dbErr)
        }
      })()
    }
    ...
    console.error("[desgroup/api/log]", err)
